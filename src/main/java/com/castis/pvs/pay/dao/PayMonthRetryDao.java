package com.castis.pvs.pay.dao;
...
@Repository("payMonthRetryDao")
@Transactional(propagation = Propagation.SUPPORTS)
public class PayMonthRetryDao {
	@PersistenceContext
	private EntityManager entityManager;


	@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
	public Serializable save(PayMonthRetry payMonthRetry)  throws Exception{
		
		if(payMonthRetry==null) { return null; }
		
		return  entityManager.unwrap(Session.class).save(payMonthRetry);
	}
	
	@SuppressWarnings("unchecked")
	public List<PayMonthRetry> selectList(Map<String, Object> paramMap) throws Exception{
		
		if(paramMap==null) {return null;}
		
		Criteria criteria = entityManager.unwrap(Session.class).createCriteria(PayMonthRetry.class);
		...
		return (List<PayMonthRetry>) list;
		
	}
	
	public PayMonthRetry selectOne(Map<String, Object> paramMap) throws Exception{
	...
	public int update(Long idx, Map<String, Object> paramMap) throws Exception {
		...
		if(paramMap==null || paramMap.isEmpty())
			...
			bf.append("update tbl_pay_month_retry                       ");
	...
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
	public void delete(PayMonthRetry payMonthRetry) throws Exception {
		entityManager.unwrap(Session.class).delete(payMonthRetry);
	...
	public int delete(Map<String, Object> paramMap)
		...
		if(paramMap==null || paramMap.isEmpty())
			...
			StringBuffer bf = new StringBuffer();
			bf.append("DELETE FROM tbl_pay_month_retry                       ");
	...
	public void update(PayMonthRetry payMonthRetry) throws Exception{
		entityManager.unwrap(Session.class).update(payMonthRetry);
	}

	@Transactional(propagation = Propagation.REQUIRED)
	public void saveOrUpdate(PayMonthRetry payMonthRetry) throws Exception{
		entityManager.unwrap(Session.class).saveOrUpdate(payMonthRetry);
	}
}
